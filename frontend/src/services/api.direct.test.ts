...
describe('API Direct Execution Tests', () => {
  ...
  it('should call agentApi.list with pagination', async () => {
    mock.onGet('/api/v1/agents/').reply(200, { agents: [], page: 2, page_size: 100 });
  ...
  it('should call agentApi.get', async () => {
    mock.onGet('/api/v1/agents/123').reply(200, { agent_id: '123', name: 'test' });
  ...
  it('should call agentApi.create', async () => {
    ...
    const { agentApi } = await import('./api');
    const result = await agentApi.create({ name: 'new', certificate_pem: 'cert' });

    expect(result.data.agent_id).toBe('456');
  });

  it('should call agentApi.update', async () => {
    mock.onPut('/api/v1/agents/123').reply(200, { agent_id: '123', name: 'updated' });
  ...
  it('should call policyApi.create', async () => {
    mock.onPost('/api/v1/policies/').reply(201, { policy_id: '999', name: 'new' });
  ...
  it('should call policyApi.update', async () => {
    mock.onPut('/api/v1/policies/789').reply(200, { policy_id: '789', name: 'updated' });
  ...
  it('should call auditApi.analytics', async () => {
    mock.onGet('/api/v1/audit/analytics').reply(200, { compliance_score: 92.5 });

    const { auditApi } = await import('./api');
    const result = await auditApi.analytics('2025-01-01T00:00:00Z', '2025-01-31T23:59:59Z');

    expect(result.data.compliance_score).toBe(92.5);
  });

  it('should call auditApi.export with CSV format', async () => {
    mock.onPost('/api/v1/audit/export').reply(200, 'data', { 'content-type': 'text/csv' });

    const { auditApi } = await import('./api');
    const result = await auditApi.export('csv', '2025-01-01T00:00:00Z', '2025-01-31T23:59:59Z');

    expect(result.status).toBe(200);
  });

  it('should call auditApi.export with JSON format', async () => {
    mock.onPost('/api/v1/audit/export').reply(200, '{}', { 'content-type': 'application/json' });

    const { auditApi } = await import('./api');
    const result = await auditApi.export('json', '2025-01-01T00:00:00Z', '2025-01-31T23:59:59Z');
