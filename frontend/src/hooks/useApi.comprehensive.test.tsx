...
describe('useApi Comprehensive Coverage', () => {
  ...
  describe('Query Keys', () => {
    it('should have correct query keys structure', () => {
      ...
      expect(hooks.queryKeys.agents.detail('123')).toEqual(['agents', 'detail', '123']);
  ...
  describe('useCreateAgent mutation', () => {
    it('should call create agent API', async () => {
      ...
      vi.mocked(apiModule.agentApi.create).mockResolvedValue(mockResponse as any);

      const { result } = renderHook(() => hooks.useCreateAgent(), { wrapper: createWrapper() });
  ...
  describe('useUpdateAgent mutation', () => {
    it('should call update agent API', async () => {
      ...
      vi.mocked(apiModule.agentApi.update).mockResolvedValue(mockResponse as any);

      const { result } = renderHook(() => hooks.useUpdateAgent(), { wrapper: createWrapper() });
  ...
  describe('useCreatePolicy mutation', () => {
    it('should call create policy API', async () => {
      ...
      vi.mocked(apiModule.policyApi.create).mockResolvedValue(mockResponse as any);

      const { result } = renderHook(() => hooks.useCreatePolicy(), { wrapper: createWrapper() });
  ...
  describe('useUpdatePolicy mutation', () => {
    it('should call update policy API', async () => {
      ...
      vi.mocked(apiModule.policyApi.update).mockResolvedValue(mockResponse as any);

      const { result } = renderHook(() => hooks.useUpdatePolicy(), { wrapper: createWrapper() });
  ...
  describe('useDeletePolicy mutation', () => {
    it('should call delete policy API', async () => {
      ...
      const { result } = renderHook(() => hooks.useDeletePolicy(), { wrapper: createWrapper() });
  ...
  describe('usePolicy query', () => {
    it('should fetch single policy', async () => {
      ...
      const { result } = renderHook(() => hooks.usePolicy('789'), { wrapper: createWrapper() });
    ...
    it('should not fetch when policyId is empty', () => {
      const { result } = renderHook(() => hooks.usePolicy(''), { wrapper: createWrapper() });
  ...
  describe('useAuditQuery query', () => {
    it('should fetch audit entries', async () => {
      ...
      vi.mocked(apiModule.auditApi.query).mockResolvedValue({ data: mockData } as any);
  ...
  describe('Query with filters', () => {
    ...
    it('should support filtering by domain', async () => {
      ...
      const { result } = renderHook(
        () => hooks.useAuditQuery({ domain: 'example.com', page: 1, page_size: 50 }),
        { wrapper: createWrapper() }
      );
