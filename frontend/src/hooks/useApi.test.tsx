...
describe("useApi hooks", () => {
  ...
  describe("useHealth", () => {
    it("should fetch health check data", async () => {
      ...
      vi.mocked(apiModule.healthApi.check).mockResolvedValue({
        data: mockData,
      } as any);
  ...
  describe("useMetrics", () => {
    it("should fetch metrics data", async () => {
      ...
      vi.mocked(apiModule.healthApi.metrics).mockResolvedValue({
        data: mockMetrics,
      } as any);
  ...
  describe("useAgents", () => {
    it("should fetch agents list", async () => {
      ...
      vi.mocked(apiModule.agentApi.list).mockResolvedValue({
        data: mockAgents,
      } as any);

      const { result } = renderHook(() => useAgents(1, 50), {
        wrapper: createWrapper(),
      });
  ...
  describe("useAgent", () => {
    it("should fetch single agent", async () => {
      ...
      vi.mocked(apiModule.agentApi.get).mockResolvedValue({
        data: mockAgent,
      } as any);
  ...
  describe("usePolicies", () => {
    it("should fetch policies list", async () => {
      ...
      vi.mocked(apiModule.policyApi.list).mockResolvedValue({
        data: mockPolicies,
      } as any);
  ...
  describe("useAuditAnalytics", () => {
    it("should fetch analytics data", async () => {
      ...
      vi.mocked(apiModule.auditApi.analytics).mockResolvedValue({
        data: mockAnalytics,
      } as any);
    ...
    it("should not fetch when times are empty", () => {
      const { result } = renderHook(() => useAuditAnalytics("", ""), {
        wrapper: createWrapper(),
      });
      expect(result.current.fetchStatus).toBe("idle");
    });
