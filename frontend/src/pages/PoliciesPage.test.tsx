...
describe('PoliciesPage', () => {
  ...
  it('should render policies page title', () => {
    vi.mocked(useApiModule.usePolicies).mockReturnValue({
      data: { policies: [], total_count: 0, page: 1, page_size: 50, has_more: false },
      isLoading: false,
      error: null,
    } as any);
    ...
    expect(screen.getByText(/manage access policies and rules/i)).toBeInTheDocument();
  });
  ...
  it('should display policies list', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText(/policy for production environments/i)).toBeInTheDocument();
      ...
      policies: [
        {
          ...
          priority: 50,
          rules: [],
          allowed_domains: ['api.example.com', 'app.example.com', 'web.example.com'],
          blocked_domains: [],
          default_action: 'deny' as const,
          is_active: true,
          created_at: '2025-01-01T00:00:00Z',
