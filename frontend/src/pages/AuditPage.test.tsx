...
describe("AuditPage", () => {
  ...
  it("should render audit page title", () => {
    ...
    expect(
      screen.getByText(/view and search audit trail entries/i),
    ).toBeInTheDocument();
  });
  ...
  it("should display error state", () => {
    ...
    render(<AuditPage />, { wrapper: createWrapper() });
    expect(
      screen.getByText(/error loading audit logs: network error/i),
    ).toBeInTheDocument();
  });
  ...
  it("should have decision filter", async () => {
    ...
    const decisionSelect = selects.find(
      (el) =>
        el.closest("div")?.querySelector("label")?.textContent === "Decision",
    );
  ...
  it("should have search functionality", () => {
    ...
    const searchButton = screen
      ...
      .find(
        (btn) =>
          btn.querySelector("svg") && btn.className.includes("bg-primary"),
      );
